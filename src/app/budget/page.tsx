...
const GRANT_RULES: Record<string, GrantRule> = {
  ...
  'nci-sbir-phase1': { id: 'nci-sbir-phase1', name: 'NCI SBIR Phase I', fullName: 'NCI Small Business Innovation Research - Phase I', maxBudget: 400000, subcontractLimit: 0.33, salaryCapPerYear: null, fringeRate: 0.30, indirectRate: 0.50, indirectBase: 'MTDC', equipmentThreshold: 5000, notes: ['NCI Exception: $400,000 (vs standard $275K)', 'Cancer-focused technology', 'Subcontracts ≤33%'], budgetEmphasis: ['Cancer therapeutic/diagnostic focus', 'Higher budget allows more R&D', 'Commercialization of cancer tech'], typicalAllocation: { personnel: '50-60%', supplies: '20-30%', subcontracts: '10-25%', equipment: '5-10%' } },
  'nci-sbir-phase2': { id: 'nci-sbir-phase2', name: 'NCI SBIR Phase II', fullName: 'NCI Small Business Innovation Research - Phase II', maxBudget: 2000000, subcontractLimit: 0.33, salaryCapPerYear: null, fringeRate: 0.30, indirectRate: 0.50, indirectBase: 'MTDC', equipmentThreshold: 5000, notes: ['NCI Exception: $2,000,000 (vs standard $1.75M)', 'Cancer product development', 'Clinical translation encouraged'], budgetEmphasis: ['Clinical-stage development', 'IND-enabling studies', 'Manufacturing scale-up'], typicalAllocation: { personnel: '45-55%', supplies: '15-25%', equipment: '10-20%', subcontracts: '15-25%' } },
            ...
            <div className="pt-2 border-t border-primary-200">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.entries(GRANT_RULES).filter(([id]) => id !== state.grantType && (
                  ...
                  (state.grantType.includes('nci') && id.includes('nci')) ||
                  (state.grantType.includes('dod') && id.includes('dod'))
                )).slice(0, 5).map(([id, rule]) => (
                  <button key={id} onClick={() => dispatch({ type: 'SET_PROJECT_INFO', payload: { grantType: id } })} className="text-xs px-2 py-1 bg-white border border-primary-300 rounded hover:bg-primary-100 text-primary-700">
                    → {rule.name} {rule.maxBudget ? `(${formatCurrency(rule.maxBudget)})` : ''}
                  </button>
                ))}
              </div>
            </div>
